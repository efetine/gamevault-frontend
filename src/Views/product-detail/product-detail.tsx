...
//!Imágenes para mostrar en el carrusel del detalle del producto(Borrar cuando se puedan obtener del back)
...
const ProductDetail: React.FC<IProduct> = ({ price, description, type, stock, name, imageUrl }) => {
  ...
  return (
    <div className={cn('flex max-w-[1120px] m-auto gap-12 mt-14 pb-14', isMobile && 'flex-col')}>
      <div className='w-[60%]'>
        <div>
          ...
          <div className='mb-5 mt-2.5 flex items-center justify-between'>
            ...
            <div className='space-y-2'>
              <div className='flex justify-between items-center'>
                <h3 className='font-semibold flex mt-6 items-center'>
                  <PackageIcon className='w-4 h-4 mr-2' />
                  Stock Availability
                </h3>
                <Badge variant={lowStock ? 'destructive' : 'secondary'}>{lowStock ? 'Low Stock' : 'In Stock'}</Badge>
              </div>
              <Progress value={stockPercentage} className='h-2' />
              <p className='text-sm text-muted-foreground'>
                {stockPercentage}% of stock remaining. {lowStock ? 'Order soon!' : ''}
              </p>
            </div>
          </div>
        </div>
        ...
        <div>
          ...
          <p className='font-normal text-gray-400 dark:text-gray-400'>{description}</p>
        </div>
      </div>
